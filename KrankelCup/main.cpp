...
void escribeArchivoTabla() {
    ...
    if (ofTabla.is_open())
    {
        for (int i = 0; i < jugJugadoresTabla.size(); i++) {
            ofTabla << jugJugadoresTabla[i].getNombre() << " " << jugJugadoresTabla[i].getPJ() << " " << jugJugadoresTabla[i].getGolesF() << " " << jugJugadoresTabla[i].getGolesC() << " " << jugJugadoresTabla[i].getPuntos();
        }
        ofTabla.close();
    }
    else cout << "Unable to open file";
    jugJugadoresTabla.clear();
